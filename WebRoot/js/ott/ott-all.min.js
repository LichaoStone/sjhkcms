...
function openShowModal(d, g, b, c) {
	...
	var a;
	if(Common.browser.chrome){
		var version =Common.browser.version;
		var arr =version.split('.')
		if(arr[0] >=19){
			var iTop = (window.screen.availHeight-30-b)/2;       //获得窗口的垂直位置;
  			var iLeft = (window.screen.availWidth-10-g)/2;           //获得窗口的水平位置;
  			window.open(d,c,'height='+b+',innerHeight='+b+',width='+g+',innerWidth='+g+',top='+iTop+',left='+iLeft+',toolbar=no,menubar=no,scrollbars=auto,resizeable=no,location=no,status=no');
		}
	}else{
		a = window.showModalDialog(d, c, f);
	}
	try {
		top.showModelDialogNumber = top.showModelDialogNumber - 1;
	} catch (h) {
...
var isSelect= false;
function isCheckAll(domName, $this) {
	if(isSelect){
		isSelect= false;
		$($this).parent().find('label').text('全选');
	}else{
		isSelect= true;
		$($this).parent().find('label').text('反选');
	}
		
	$("input[name='"+domName+"']").each(function(){ 
...
function changeNavigation(obj, dom){
	if (obj.value == '') {
	...
	$.get(window.ctx + "/common/changeNavigation.do" , param,
			function(res) {
				if (res == null) {
					return;
				}
...
function changeChannel2(obj, dom){
	if (obj.value == '') {
		jQuery('#' + dom + ' option').remove();
		jQuery('#' + dom)
				.append("<option value=''>=\u8bf7\u9009\u62e9=</option>");
	...
	$.getJSON(window.ctx + "/channel/changeChannel2.do" , param,
			function(res) {
				if (res == null) {
					return;
				}
				...
				$.each(res, function(i,channel) {
					var children=channel.children;
					 $("#" + dom).append("<option data='"+channel.name+"' value='" + channel.id
							 + "'>" + channel.name+ "</option>");
					 if (children != null) {
						 $.each(children, function(j,channel2) {
							 $("#" + dom).append("<option data='"+channel2.name+"' value='" + channel2.id
									 + "'>" +"  ---"+ channel2.name + "</option>");
						 });
					 }
				});
	});
